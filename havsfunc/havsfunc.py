...
from vsdenoise import DFTTest, bm3d, nl_means, prefilter_to_full_range
...
def QTGMC(
    ...
    Parameters:
        ...
        vulkan: Whether to use the vulkan version of NNEDI3 and EEDI3.
    """
    ...
    Search = fallback(Search,             [ 3,        2,        2,        2,        2,        2,        2,        2,        0,        0,        0      ][pNum])
    ...
    sharpMul = (
        2 if temporalSL else 1.5 if spatialSL else 1
    ...
    if (
        EZDenoise is not None
        and EZDenoise > 0
        and EZKeepGrain is not None
        and EZKeepGrain > 0
    ):
    ...
    if NoiseProcess is None:
        if EZDenoise is not None and EZDenoise > 0:
            NoiseProcess = 1
        elif (EZKeepGrain is not None and EZKeepGrain > 0) or Preset in [
            "placebo",
            "very slow",
        ]:
            NoiseProcess = 2
    ...
    noiseCentre = (
        scale_value(128.5, 8, bits)
        if Denoiser in ["fft3df", "fft3dfilter"]
        else neutral
    )
    ...
    if (
        ShutterAngleOut * FPSDivisor == ShutterAngleSrc
    ):  # If motion blur output is same as input
        ShutterBlur = 0
    ...
    if not isinstance(srchClip, vs.VideoNode):
        ...
        if TR0 > 0:  # -2    -1    0     1     2
            ts1 = bobbed.std.AverageFrames(
                [1] * 3, scenechange=True, planes=CMplanes
            )  # 0.00  0.33  0.33  0.33  0.00
        if TR0 > 1:
            ts2 = bobbed.std.AverageFrames(
                [1] * 5, scenechange=True, planes=CMplanes
    ...
    if isinstance(srchClip, vs.VideoNode):
        ...
        binomial0 = core.std.Merge(
            ts1, bobbed, weight=0.25 if ChromaMotion or is_gray else [0.25, 0]
        )
    else:
        binomial0 = core.std.Merge(
            core.std.Merge(
                ts1, ts2, weight=0.357 if ChromaMotion or is_gray else [0.357, 0]
            ),
            bobbed,
    ...
    if isinstance(srchClip, vs.VideoNode) or Rep0 <= 0:
        ...
        repair0 = QTGMC_KeepOnlyBobShimmerFixes(
            binomial0, bobbed, Rep0, RepChroma and ChromaMotion
        )
    ...
    if not isinstance(srchClip, vs.VideoNode):
        if SrchClipPP == 1:
            ...
            spatialBlur = gauss_blur(
                repair0.std.Convolution(matrix=matrix, planes=CMplanes), 1.75
            )
            spatialBlur = core.std.Merge(
                spatialBlur,
                repair0,
                weight=0.1 if ChromaMotion or is_gray else [0.1, 0],
            )
        if SrchClipPP <= 0:
            srchClip = repair0
        elif SrchClipPP < 3:
            ...
            tweaked = core.std.Expr(
                [repair0, bobbed], expr=expr if ChromaMotion or is_gray else [expr, ""]
            )
            expr = "x {i7} + y < x {i2} + x {i7} - y > x {i2} - x 51 * y 49 * + 100 / ? ?".format(
                i7=scale_value(7, 8, bits), i2=scale_value(2, 8, bits)
            )
            srchClip = core.std.Expr(
                [spatialBlur, tweaked],
                expr=expr if ChromaMotion or is_gray else [expr, ""],
            )
    ...
    if maxTR > 0:
        if not isinstance(srchSuper, vs.VideoNode):
            srchSuper = srchClip.mvu.Super(**super_args)
        if not isinstance(bVec1, vs.VideoNode):
            bVec1 = srchSuper.mvu.Analyse(delta=-1, **analyse_args)
            if RefineMotion:
                bVec1 = core.mvu.Recalculate(srchSuper, bVec1, **recalculate_args)
        if not isinstance(fVec1, vs.VideoNode):
            fVec1 = srchSuper.mvu.Analyse(delta=1, **analyse_args)
            if RefineMotion:
                fVec1 = core.mvu.Recalculate(srchSuper, fVec1, **recalculate_args)
    if maxTR > 1:
        if not isinstance(bVec2, vs.VideoNode):
            bVec2 = srchSuper.mvu.Analyse(delta=-2, **analyse_args)
            if RefineMotion:
                bVec2 = core.mvu.Recalculate(srchSuper, bVec2, **recalculate_args)
        if not isinstance(fVec2, vs.VideoNode):
            fVec2 = srchSuper.mvu.Analyse(delta=2, **analyse_args)
            if RefineMotion:
                fVec2 = core.mvu.Recalculate(srchSuper, fVec2, **recalculate_args)
    if maxTR > 2:
        if not isinstance(bVec3, vs.VideoNode):
            bVec3 = srchSuper.mvu.Analyse(delta=-3, **analyse_args)
            if RefineMotion:
                bVec3 = core.mvu.Recalculate(srchSuper, bVec3, **recalculate_args)
        if not isinstance(fVec3, vs.VideoNode):
            fVec3 = srchSuper.mvu.Analyse(delta=3, **analyse_args)
            if RefineMotion:
                fVec3 = core.mvu.Recalculate(srchSuper, fVec3, **recalculate_args)
    ...
    if NoiseTR > 0:
        fullSuper = fullClip.mvu.Super(onelevel=True, **super_args)  # TEST chroma OK?
    ...
    if NoiseProcess > 0:
        ...
        if Denoiser == "bm3d":
            ...
            dnWindow = DFTTest(sigma=Sigma * 4).denoise(
                noiseWindow, tr=NoiseTR, planes=CNplanes
            )
        elif Denoiser in ["knlm", "knlmeanscl"]:
            dnWindow = nl_means(
                noiseWindow, strength=Sigma, tr=NoiseTR, planes=CNplanes
            )
        else:
            dnWindow = noiseWindow.fft3dfilter.FFT3DFilter(
                sigma=Sigma, planes=CNplanes, bt=noiseTD, ncpu=FftThreads
            )
        ...
        if totalRestore > 0:
            ...
            if InputType > 0:
                ...
                deintNoise = noise.resize.Bob(
                    tff=TFF, filter_param_a=0, filter_param_b=0.5
                )
            elif NoiseDeint == "generate":
                deintNoise = QTGMC_Generate2ndFieldNoise(
                    noise, denoised, ChromaNoise, TFF
                )
            else:
            ...
            if StabilizeNoise:
                noiseSuper = deintNoise.mvu.Super(onelevel=True, **super_args)
                mcNoise = core.mvu.Compensate(
                    deintNoise, noiseSuper, bVec1, thscd1=ThSCD1, thscd2=ThSCD2
                )
                expr = (
                    f"x {neutral} - abs y {neutral} - abs > x y ? 0.6 * x y + 0.2 * +"
                )
                finalNoise = core.std.Expr(
                    [deintNoise, mcNoise],
    ...
    if InputType > 1:
        ediInput = (
            innerClip.std.SeparateFields(tff=TFF)
            .std.SelectEvery(cycle=4, offsets=[0, 3])
            .std.DoubleWeave(TFF)[::2]
        )
    else:
    ...
    if InputType < 2:
        ...
        inputTypeBlend = core.mvu.SADMask(srchClip, bVec1, ml=ProgSADMask)
    ...
    if TR1 > 0 or temporalSL:
        ediSuper = edi.mvu.Super(onelevel=True, **super_args)
    if temporalSL:
        bComp1 = core.mvu.Compensate(edi, ediSuper, bVec1, thscd1=ThSCD1, thscd2=ThSCD2)
        fComp1 = core.mvu.Compensate(edi, ediSuper, fVec1, thscd1=ThSCD1, thscd2=ThSCD2)
        tMax = core.std.Expr(
            [core.std.Expr([edi, fComp1], expr="x y max"), bComp1], expr="x y max"
        )
        tMin = core.std.Expr(
            [core.std.Expr([edi, fComp1], expr="x y min"), bComp1], expr="x y min"
        )
        if SLRad > 1:
            bComp3 = core.mvu.Compensate(
                edi, ediSuper, bVec3, thscd1=ThSCD1, thscd2=ThSCD2
            )
            fComp3 = core.mvu.Compensate(
                edi, ediSuper, fVec3, thscd1=ThSCD1, thscd2=ThSCD2
            )
            tMax = core.std.Expr(
                [core.std.Expr([tMax, fComp3], expr="x y max"), bComp3], expr="x y max"
            )
            tMin = core.std.Expr(
                [core.std.Expr([tMin, fComp3], expr="x y min"), bComp3], expr="x y min"
            )
    ...
    if SVThin > 0:
        ...
        neighborD = core.std.Expr(
            [vertMedD, vertMedD.std.Convolution(matrix=matrix, planes=0)],
            expr=expr if is_gray else [expr, ""],
        )
    ...
    if SLMode == 1:
        if SLRad <= 1:
            ...
            sharpLimit1 = core.zsmooth.Repair(
                backBlend1, core.zsmooth.Repair(backBlend1, edi, mode=12), mode=1
            )
    elif SLMode == 2:
    ...
    if GrainRestore <= 0:
        ...
        addNoise1 = core.std.MergeDiff(
            backBlend2,
            finalNoise.std.Expr(expr=expr if ChromaNoise or is_gray else [expr, ""]),
            planes=CNplanes,
        )

    # Final light linear temporal smooth for denoising
    if TR2 > 0:
        stableSuper = addNoise1.mvu.Super(onelevel=True, **super_args)
    if TR2 <= 0:
        stable = addNoise1
    elif TR2 == 1:
        stable = core.mvu.Degrain1(
            addNoise1,
            stableSuper,
            [bVec1, fVec1],
            thsad=ThSAD2,
        ...
        stable = core.mvu.Degrain2(
            addNoise1,
            stableSuper,
            [bVec1, fVec1, bVec2, fVec2],
            thsad=ThSAD2,
            thscd1=ThSCD1,
            thscd2=ThSCD2,
        )
    else:
        stable = core.mvu.Degrain3(
            addNoise1,
            stableSuper,
            [bVec1, fVec1, bVec2, fVec2, bVec3, fVec3],
            thsad=ThSAD2,
            thscd1=ThSCD1,
            thscd2=ThSCD2,
        )
    ...
    if SLMode == 3:
        if SLRad <= 1:
            ...
            sharpLimit2 = core.zsmooth.Repair(
                repair2, core.zsmooth.Repair(repair2, edi, mode=12), mode=1
            )
    elif SLMode >= 4:
    ...
    if ShutterBlur > 1:
        ...
        sbBVec1 = core.mvu.Recalculate(srchSuper, bVec1, **recalculate_args)
        sbFVec1 = core.mvu.Recalculate(srchSuper, fVec1, **recalculate_args)
    elif ShutterBlur > 0:
        sbBVec1 = bVec1
        sbFVec1 = fVec1
    ...
    if ShutterBlur > 0:
        sblurSuper = addNoise2.mvu.Super(onelevel=True, **super_args)
        sblur = core.mvu.FlowBlur(
            addNoise2,
            sblurSuper,
            sbBVec1,
            sbFVec1,
    ...
    if ShutterBlur <= 0:
        ...
        sbMotionMask = core.mvu.VectorLengthMask(srchClip, bVec1, ml=SBlurLimit)
    ...
    if ShowNoise <= 0:
        ...
        output = finalNoise.std.Expr(
            expr=expr if ChromaNoise or is_gray else [expr, repr(neutral)]
        )
...
def QTGMC_Interpolate(
    ...
    if vulkan:
        nnedi3 = partial(core.nnedi3vk.NNEDI3, field=field, **nnedi3_args)
        eedi3 = partial(
            core.eedi3vk2.EEDI3,
            field=field,
            planes=planes,
        ...
        eedi3 = partial(
            core.vszip.EEDI3, field=field, planes=planes, mdis=EdiMaxD, **eedi3_args
        )
    ...
    if InputType == 1:
        ...
        interp = eedi3(
            Input,
            sclip=nnedi3(
                Input, planes=planes, nsize=NNSize, nns=NNeurons, qual=EdiQual
            ),
        )
    elif EdiMode == "eedi3":
        interp = eedi3(Input)
        ...
        interp = fallback(
            Fallback, Input.resize.Bob(tff=TFF, filter_param_a=0, filter_param_b=0.5)
        )
...
def QTGMC_Generate2ndFieldNoise(
    ...
    noiseMax = origNoise.std.Maximum(planes=planes).std.Maximum(
        planes=planes, coordinates=[0, 0, 0, 1, 1, 0, 0, 0]
    )
    noiseMin = origNoise.std.Minimum(planes=planes).std.Minimum(
        planes=planes, coordinates=[0, 0, 0, 1, 1, 0, 0, 0]
    )
    random = (
...
def QTGMC_MakeLossless(
    Input: vs.VideoNode, Source: vs.VideoNode, InputType: int, TFF: bool | None = None
) -> vs.VideoNode:
    """
    ...
    if InputType <= 0:
        ...
        srcFields = Source.std.SeparateFields(tff=TFF).std.SelectEvery(
            cycle=4, offsets=[0, 3]
        )
    newFields = Input.std.SeparateFields(tff=TFF).std.SelectEvery(
        cycle=4, offsets=[1, 2]
    )
    processed = (
    ...
    vmNewDiff1 = vertMedDiff.std.SeparateFields(tff=TFF).std.SelectEvery(
        cycle=4, offsets=[1, 2]
    )
    ...
    vmNewDiff3 = core.zsmooth.Repair(
        vmNewDiff2, vmNewDiff2.zsmooth.RemoveGrain(mode=2), mode=1
    )
...
def QTGMC_ApplySourceMatch(
    ...
    errorAdjust1 = [
        1.0,
        2.0 / (1.0 + errorTemporalSimilarity),
        8.0 / (3.0 + 5.0 * errorTemporalSimilarity),
    ][MatchTR1]
    if SourceMatch < 1 or InputType == 1:
        match1Clip = Deinterlace
    else:
        match1Clip = (
            Deinterlace.std.SeparateFields(tff=TFF)
            .std.SelectEvery(cycle=4, offsets=[0, 3])
            .std.DoubleWeave(TFF)[::2]
        )
    if SourceMatch < 1 or MatchTR1 <= 0:
        ...
        match1Update = core.std.Expr(
            [Source, match1Clip], expr=f"x {errorAdjust1 + 1} * y {errorAdjust1} * -"
        )
    if SourceMatch > 0:
        ...
        if MatchTR1 > 0:
            match1Super = match1Edi.mvu.Super(onelevel=True, **super_args)
            match1Degrain1 = core.mvu.Degrain1(
                match1Edi,
                match1Super,
                [bVec1, fVec1],
                thsad=ThSAD1,
        ...
        if MatchTR1 > 1:
            match1Degrain2 = core.mvu.Degrain1(
                match1Edi,
                match1Super,
                [bVec2, fVec2],
                thsad=ThSAD1,
    ...
    if SourceMatch < 2 or InputType == 1:
        ...
        match2Clip = (
            match1Shp.std.SeparateFields(tff=TFF)
            .std.SelectEvery(cycle=4, offsets=[0, 3])
            .std.DoubleWeave(TFF)[::2]
        )
    if SourceMatch > 1:
        ...
        if MatchTR2 > 0:
            match2Super = match2Edi.mvu.Super(onelevel=True, **super_args)
            match2Degrain1 = core.mvu.Degrain1(
                match2Edi,
                match2Super,
                [bVec1, fVec1],
                thsad=ThSAD1,
        ...
        if MatchTR2 > 1:
            match2Degrain2 = core.mvu.Degrain1(
                match2Edi,
                match2Super,
                [bVec2, fVec2],
                thsad=ThSAD1,
    ...
    if SourceMatch < 3 or MatchTR2 <= 0:
        ...
        match3Update = core.std.Expr(
            [match2Edi, match2], expr=f"x {errorAdjust2 + 1} * y {errorAdjust2} * -"
        )
    if SourceMatch > 2:
        if MatchTR2 > 0:
            match3Super = match3Update.mvu.Super(onelevel=True, **super_args)
            match3Degrain1 = core.mvu.Degrain1(
                match3Update,
                match3Super,
                [bVec1, fVec1],
                thsad=ThSAD1,
        ...
        if MatchTR2 > 1:
            match3Degrain2 = core.mvu.Degrain1(
                match3Update,
                match3Super,
                [bVec2, fVec2],
                thsad=ThSAD1,
